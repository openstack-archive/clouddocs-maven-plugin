...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	exclude-result-prefixes="wadl rax d xhtml xsdxt" version="2.0">
	...
	<xsl:param name="wadl.noreqresp.msg">
		<para>This operation does not require a request body and does
			not return a response body.</para>
	</xsl:param>
	...
	<xsl:template match="d:*[@role = 'api-reference']">
		...
		<xsl:element name="{name(.)}">
			...
			<xsl:apply-templates
				select="d:*[not(self::d:section)]|processing-instruction()"/>
			<!-- 
	...
	<xsl:template match="wadl:method" name="method-row"
		mode="method-rows">
		...
		<tr>
			...
			<td>
				...
				<xsl:variable name="path">
					<xsl:choose>
						<xsl:when
							test="xs:integer($trim.wadl.uri.count) &gt; 0">
							<xsl:call-template name="trimUri">
								<xsl:with-param name="trimCount"
								select="$trim.wadl.uri.count"/>
								<xsl:with-param name="uri"
								select="parent::wadl:resource/@path"
								/>
							</xsl:call-template>
						</xsl:when>
						<xsl:otherwise>
			...
			<td>
				<xsl:choose>
					<xsl:when test="wadl:doc//*[@role='shortdesc']">
						<para>
							<xsl:apply-templates
								select="(wadl:doc//*[@role = 'shortdesc'])[1]"
								mode="process-shortdesc"/>
						</para>
					</xsl:when>
					<xsl:otherwise>
						<xsl:apply-templates select="wadl:doc"
							mode="process-no-shortdesc"/>
					</xsl:otherwise>
				</xsl:choose>
			</td>
		</tr>
	...
	<xsl:template match="wadl:resources" name="wadl-resources">
					...
					<xsl:if test="processing-instruction('rax-wadl')">
						<xsl:call-template name="pi-attribute">
							<xsl:with-param name="pis"
								select="processing-instruction('rax-wadl')"/>
							<xsl:with-param name="attribute"
								select="'skipSummary'"/>
						</xsl:call-template>
					</xsl:if>
					<xsl:if test="parent::*[@role = 'api-reference']"
						>true</xsl:if>
				</xsl:with-param>
		...
		<xsl:variable name="skipSummary"
			select="boolean(number($skipSummaryN))"/>
	...
	<xsl:template match="wadl:method">
		<xsl:param name="sectionId"
			select="ancestor::d:section[1]/@xml:id"/>
		...
		<xsl:variable name="skipNoRequestTextN">
			<xsl:call-template name="makeBoolean">
				<xsl:with-param name="boolValue">
					<xsl:if
						test="ancestor-or-self::*/processing-instruction('rax-wadl')|processing-instruction('rax-wadl')">
						<xsl:call-template name="pi-attribute">
							<xsl:with-param name="pis"
								select="ancestor-or-self::*/processing-instruction('rax-wadl')"/>
							<xsl:with-param name="attribute"
								select="'skipNoRequestText'"/>
						</xsl:call-template>
					</xsl:if>
				</xsl:with-param>
			</xsl:call-template>
		...
		<xsl:variable name="skipNoResponseTextN">
			<xsl:call-template name="makeBoolean">
				<xsl:with-param name="boolValue">
					<xsl:if
						test="ancestor-or-self::*/processing-instruction('rax-wadl')">
						<xsl:call-template name="pi-attribute">
							<xsl:with-param name="pis"
								select="ancestor-or-self::*/processing-instruction('rax-wadl')"/>
							<xsl:with-param name="attribute"
								select="'skipNoResponeText'"/>
						</xsl:call-template>
					</xsl:if>
				</xsl:with-param>
			</xsl:call-template>
		...
		<xsl:variable name="addMethodPageBreaks"
			select="boolean(number($addMethodPageBreaksN))"/>
		...
		<xsl:variable name="method.title">
			<xsl:choose>
				...
				<xsl:when test="@id or @rax:id">
					<xsl:message>Warning: No title found for
						wadl:method</xsl:message>
		...
		-->
		<xsl:variable name="raxid"
			select="if (@rax:id) then @rax:id else @id"/>
		<!-- generate id from raxid -->
		<xsl:variable name="app_raxid" select="generate-id($raxid)"/>
		<xsl:variable name="sectionIdComputed"
			select="concat(@name,'_',$app_raxid,'_',$raxid,'_',translate(parent::wadl:resource/@path, $replacechars, '___'),'_',$sectionId)"/>
		...
		<section xml:id="{$sectionIdComputed}">
			<xsl:processing-instruction name="dbhtml"
				>stop-chunking</xsl:processing-instruction>
			<title>
				<xsl:value-of select="$method.title"/>
			</title>
			<xsl:if
				test="$sectionIdComputed != concat(@name,'_',$app_raxid,'_',$raxid,'_',translate(parent::wadl:resource/@path, $replacechars, '___'),'_')">
				<anchor
					xml:id="{concat(@name,'_',$app_raxid,'_',$raxid,'_',translate(parent::wadl:resource/@path, $replacechars, '___'),'_')}"
					xreflabel="{$method.title}"/>
			</xsl:if>
			<xsl:if test="$security = 'writeronly'">
				<para security="writeronly">Source wadl: <link
						xlink:href="{@rax:original-wadl}"
							><xsl:value-of select="@rax:original-wadl"
						/></link> (method id: <xsl:value-of
						select="@rax:id"/>)</para>
			</xsl:if>

			<!-- Method Docs -->
			<xsl:choose>
				<xsl:when test="wadl:doc//db:*[@role = 'shortdesc']"
			...
			<xsl:choose>
				<xsl:when
					test="wadl:response[not(starts-with(normalize-space(@status),'2') or starts-with(normalize-space(@status),'3'))]/wadl:doc">
					<para>This table shows the possible response codes
					...
					<informaltable rules="all" width="100%">
						...
						<tbody>
							<xsl:apply-templates
								select="wadl:response[@status and (starts-with(@status,'2') or starts-with(@status,'3'))]"
								mode="responseTable">
								<xsl:sort select="@status"/>
							</xsl:apply-templates>
							<xsl:apply-templates
								select="wadl:response[@status and not(starts-with(@status,'2') or starts-with(@status,'3'))]"
								mode="responseTable">
								<xsl:sort select="@status"/>
							</xsl:apply-templates>
							<xsl:apply-templates
				...
				<xsl:otherwise>
					<xsl:if
						...
						<simpara>
							<emphasis role="bold">Normal response
								codes: </emphasis>
							<xsl:apply-templates
								select="wadl:response"
					...
					<xsl:if
						...
						<simpara>
							<emphasis role="bold">Error response
								codes: </emphasis>
							<!--
			...
			<xsl:variable name="requestSection">
				...
				<xsl:call-template name="paramTable">
					...
					<xsl:if
						test="not($skipNoRequestText) and (not(wadl:request) or wadl:request[wadl:representation[@mediaType = 'application/xml' and not(@element)]])">
			...
			<xsl:variable name="responseSection">
				<section xml:id="{$sectionIdComputed}-Response">
					...
					<xsl:if
						test="wadl:response/wadl:param[@style = 'header']">
						<xsl:call-template name="paramTable">
							<xsl:with-param name="mode"
								select="'Response'"/>
							<xsl:with-param name="method.title"
					...
					<xsl:if
						test="not($skipNoResponseText) and not(wadl:response[starts-with(normalize-space(@status),'2')]/wadl:representation)">
			...
			<xsl:if
				test="$requestSection//d:section/*[not(self::d:title)]">
				<xsl:copy-of select="$requestSection"/>
			</xsl:if>
			<xsl:if
				test="$responseSection//d:section/*[not(self::d:title)]">
	...
	<xsl:template match="wadl:response" mode="responseTable">
		<tr>
			<td align="left">
				<xsl:value-of
					select="if(@status) then @status else '400 500 &#x2026;'"
				/>
			</td>
			<td align="left">
				<xsl:value-of select="wadl:doc/@title"/>
	...
	<xsl:template match="wadl:representation">
		...
		<xsl:variable name="plainParams">
			<xsl:choose>
				<xsl:when
					test="wadl:param[@style = 'plain'] and contains(@mediaType,'json')">
					<xsl:call-template name="paramList">
						<xsl:with-param name="mode"
							select="if(ancestor::wadl:response) then 'Response' else 'Request'"/>
						<xsl:with-param name="method.title"
							select="$method.title"/>
					</xsl:call-template>
				</xsl:when>
	...
	<xsl:template match="wadl:doc" mode="representation">
		...
		<!-- a default title will be computed below in this case -->
		<xsl:variable name="title-calculated">
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:if
						test="ancestor::wadl:method/wadl:doc/@title">
						<xsl:value-of
							select="ancestor::wadl:method/wadl:doc/@title"
						/>
					</xsl:if>
					<xsl:choose>
					...
					<xsl:choose>
						<xsl:when test="$type = 'application/xml'">:
							XML</xsl:when>
						<xsl:when test="$type = 'application/json'">:
							JSON</xsl:when>
						<xsl:when
							test="$type = 'application/atom+xml'">:
							ATOM</xsl:when>
		...
		<xsl:choose>
			<xsl:when test=".//xsdxt:samples">
				<xsl:apply-templates
					select=".//xsdxt:samples/xsdxt:sample"
					mode="sample"/>
			</xsl:when>
			...
			<xsl:when test=".//xsdxt:code">
				<example>
					<title>
						<xsl:value-of select="$title-calculated"/>
					</title>
					<xsl:apply-templates mode="sample"/>
				</example>
			</xsl:when>
	...
	<xsl:template match="wadl:param">
		...
		<xsl:variable name="default.param.type"
			select="if (@style != 'plain') then 'String' else ''"/>
		<xsl:variable name="type">
			<xsl:value-of
				select="if (@type and contains(@type, ':')) then substring-after(@type,':') else if(@type) then @type else $default.param.type"
			/>
		</xsl:variable>
		...
		<xsl:variable name="jsonPathDepth"
			select="(string-length(@path) - string-length(translate(@path,'.',''))) - 1"/>
		<tr>
			<td align="left">
				<xsl:choose>
					<xsl:when
						test="$style = 'plain' and contains(parent::wadl:representation/@mediaType, 'json') and ends-with(@path,'[*]')">
						<para>
							<xsl:if
								test="$style = 'plain' and $jsonPathDepth &gt; 0">
								<xsl:for-each
								select="1 to $jsonPathDepth"
								>&#160;&#187;&#160;</xsl:for-each>
							</xsl:if>
							<emphasis>
								<code role="hyphenate-true">
								<xsl:value-of select="@name"/>
								</code>
							</emphasis>
						</para>
					...
					<xsl:when
						test="$style = 'plain' and contains(parent::wadl:representation/@mediaType, 'json')">
						<para>
							<xsl:if
								test="$style = 'plain' and $jsonPathDepth &gt; 0">
								<xsl:for-each
					...
					<xsl:otherwise>
						<para>
							<code role="hyphenate-true">
								<xsl:value-of
								select="concat(if (@style = 'template') then '{' else '', @name, if (@style = 'template') then '}' else '')"
								/>
							</code>
						</para>
					</xsl:otherwise>
			...
			<td>
				<xsl:choose>
					<xsl:when
						test="not(wadl:doc/d:para) and not(wadl:doc/d:formalpara) and not(wadl:doc/d:itemizedlist)">
						<para>
							<xsl:apply-templates
								select="wadl:doc/node()"/>
						</para>
					</xsl:when>
					<xsl:otherwise>
						<xsl:apply-templates select="wadl:doc/*"/>
				...
				<xsl:if test="wadl:option or @style != 'template'">
					<para>
						<xsl:if test="wadl:option"> Possible values:
							...
							</xsl:for-each> Default: <xsl:value-of
								select="@default"
							/><xsl:text>. </xsl:text>
						</xsl:if>
					</para>
				...
				<xsl:if
					test="@style = 'plain' and @path and contains(parent::wadl:representation/@mediaType, 'json')">
					<para>JSONPath: <code><xsl:value-of select="@path"
							/></code></para>
				</xsl:if>
			</td>
		</tr>
	...
	<xsl:template name="paramTable">
		...
		<xsl:variable name="tableType"
			select="(: if($style = 'plain') then 'informaltable' else :)'informaltable'"/>
		<xsl:if test="$mode='Request' or $mode='Response'">

			<para>This table shows the <xsl:value-of
					select="$styleLowercase"/> parameters for the
					<xsl:value-of
					select="concat($method.title, ' ', $mode)"
				/>:</para>
			<xsl:element name="{$tableType}">
				...
				<xsl:if test="$tableType = 'table'">
					<caption>
						<xsl:value-of
							select="concat($method.title,' ',$mode, ' ', $styleLowercase, ' parameters')"
						/>
					</caption>
				</xsl:if>
				<col width="30%"/>
	...
	<xsl:template name="paramList">
		...
		<xsl:param name="method.title"/>
		<xsl:variable name="plainParams"
			select="wadl:param[@style = 'plain' and ./wadl:doc and @path]"/>
		<xsl:if test="$plainParams">
			<para>This list shows the body parameters for the
					<xsl:value-of
					select="concat($method.title, ' ', $mode)"
				/>:</para>
			<itemizedlist role="paramList">
				<xsl:call-template name="group-params">
					<xsl:with-param name="plainParams"
						select="$plainParams"/>
	...
	<xsl:template name="group-params">
		...
		<xsl:for-each-group select="$plainParams"
			group-by="tokenize(substring-after(@path,'$.'),'\.')[$token-number]">
			<xsl:variable name="path"
				select="concat('$.',replace(string-join(for $item in tokenize(substring-after(current-group()[1]/@path,'$.'),'\.')[position() &lt; ($token-number + 1)] return concat($item,'.'),''),'(.*)\.$','$1'))"/>
			<xsl:variable name="current-param"
				select="current-group()[@path = $path]"/>
			<xsl:variable name="optionality">
				<xsl:value-of
					select="if ($current-param/@required = 'true') then 'Required. ' else if ($current-param/@required = 'false') then 'Optional. ' else ''"
				/>
			</xsl:variable>
			<xsl:choose>
				<xsl:when
					test="current-grouping-key() = '[*]' and not($top)">
					<xsl:variable name="children">
						<xsl:call-template name="group-params">
							<xsl:with-param name="token-number"
				...
				<xsl:otherwise>
					<listitem role="body-params">
						<para role="paramList"><emphasis role="bold"
								><xsl:value-of
								select="current-grouping-key()"
								/></emphasis>: <xsl:value-of
								select="if($current-param/@type) then concat(upper-case(substring(@type,1,1)),substring(@type,2),'. ') else if(current-grouping-key() = '[*]') then 'Array. ' else ''"/>
							<xsl:if test="not($optionality = '')"
								><xsl:value-of
								select="$optionality"/></xsl:if>
						</para>
						<xsl:choose>
							<xsl:when
								test="$current-param/wadl:doc/d:para or $current-param/wadl:doc/d:itemizedlist or $current-param/wadl:doc/d:orderedlist or $current-param/wadl:doc/d:formalpara or $current-param/wadl:doc/d:simpara">
								<xsl:apply-templates
								select="$current-param/wadl:doc/*"
								mode="copy"/>
							</xsl:when>
							<xsl:otherwise>
								<para>
								<xsl:apply-templates
								select="$current-param/wadl:doc/node()"
								mode="copy"/>
								</para>
							</xsl:otherwise>
						</xsl:choose>
	...
	<xsl:template name="pi-attribute">
		<xsl:param name="pis"
			select="processing-instruction('BOGUS_PI')"/>
