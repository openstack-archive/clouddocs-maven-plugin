...
ERROR: Feedback email not set but internal comments are enabled.
    ...
    <xsl:if test="$branding = 'openstack' and $social.icons = '0'">
      <div id="gplusone">
        <a id="AskOOLink1" href="http://ask.openstack.org" target="_blank" title="Questions? Discuss on ask.openstack.org"><i class="icon-question"><xsl:comment/></i></a> <xsl:comment/>
