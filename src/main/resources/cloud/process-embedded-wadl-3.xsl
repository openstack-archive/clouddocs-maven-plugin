...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:param name="wadl.noreqresp.msg">
		<para>This operation does not accept a request body or return
	...
	<xsl:template match="wadl:method" name="method-row"
		...
		<xsl:variable name="app_raxid"
			select="if(ancestor::wadl:resources/@xml:id) then concat(ancestor::wadl:resources/@xml:id, '_') else ''"/>
			...
			<td>
				...
				<xsl:variable name="path">
					<xsl:choose>
						...
						<xsl:otherwise>
							<xsl:value-of
								select="if(not(starts-with(parent::wadl:resource/@path,'/'))) then concat('/', parent::wadl:resource/@path) else parent::wadl:resource/@path"
							/>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:variable>
	...
	<xsl:template match="wadl:method">
		...
		<section xml:id="{$sectionIdComputed}">
			...
			<xsl:if
				test="$sectionIdComputed != concat(@name,'_',$app_raxid,$raxid,'_',translate(parent::wadl:resource/@path, $replacechars, '___'),'_')">
				<anchor
					xml:id="{concat(@name,'_',$app_raxid,$raxid,'_',translate(parent::wadl:resource/@path, $replacechars, '___'),'_')}"
	...
	<xsl:template name="paramTable">
		...
		<xsl:if test="$mode='request' or $mode='response'">
			<para>This table shows the <xsl:value-of
					...
					<xsl:value-of select="lower-case($method.title)"/>
				<xsl:value-of select="concat(' ', $mode)"/>:</para>
	...
	<xsl:template name="paramList">
		...
		<xsl:if test="$plainParams">
			<para>This list shows the body parameters for the
					<xsl:value-of
					select="concat($method.tableintro, ' ', $mode)"
				/>:</para>
	...
	<xsl:template name="group-params">
		...
		<xsl:for-each-group select="$plainParams"
			...
			<xsl:variable name="param-name">
				<xsl:value-of select="($current-param/@name)"/>
			</xsl:variable>
			<xsl:message>Parameter name is <xsl:value-of
