...
public class CopyAndTransform extends DefaultStep {
    ...
    public void run() throws SaxonApiException {
        ...
        if (!"file".equals(uri.getScheme())) {
            getLog().error("Step: '"+step.getNode().getNodeName() + "' - DocBook File: '" +inputFileName+ "' - File: '" +uri.getPath()+ "' - Problem: Only 'file' scheme URIs are supported by this step!");
        ...
        if (!file.exists()) {
            getLog().error("Step: '"+step.getNode().getNodeName() + "' - DocBook File: '" +inputFileName+ "' - File: '" +uri.getPath()+ "' - Problem: File does not exist!");
            throw new XProcException(step.getNode(), "Cannot copy: file does not exist: " + file.getAbsolutePath());
        }

        if (file.isDirectory()) {
        	getLog().error("Step: '"+step.getNode().getNodeName() + "' - DocBook File: '" +inputFileName+ "' - File: '" +uri.getPath()+ "' - Problem: File is a directory!");
        ...
        try {
            ...
            if(ext.equalsIgnoreCase("svg")) {
            	...
            	if(!check) {
                	getLog().error("Step: '"+step.getNode().getNodeName() + "' - DocBook File: '" +inputFileName+ "' - File: '" +uri.getPath()+ "' - Problem: Could not transform SVG to PNG!");
