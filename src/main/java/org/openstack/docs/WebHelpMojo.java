...
import org.openstack.docs.builders.PDFBuilder;
...
public abstract class WebHelpMojo extends AbstractWebhelpMojo {
   ...
   transformer.setParameter("compute.wadl.path.from.docbook.path",computeWadlPathFromDocbookPath);
	...
	//FileUtils.extractJaredDirectory("apiref",ApiRefMojo.class,targetDirectory);
	...
	if(null != webhelpWar && !"0".equals(webhelpWar)){
		...
		org.openstack.docs.FileUtils.addDirectory(zout, sourceDir);
    ...
    protected void copyTemplate(File result) throws MojoExecutionException {
        ...
        org.openstack.docs.FileUtils.extractJaredDirectory("content", WebHelpMojo.class, targetDirectory);
        org.openstack.docs.FileUtils.extractJaredDirectory("common", WebHelpMojo.class, targetDirectory);
    ...
    public void preProcess() throws MojoExecutionException {
        ...
        if (!targetDirectory.exists()) {
            org.openstack.docs.FileUtils.mkdir(targetDirectory);
        }

        //
        // Extract all images into the image directory.
        //
        org.openstack.docs.FileUtils.extractJaredDirectory("cloud/war", PDFMojo.class, xslParentDirectory);
        org.openstack.docs.FileUtils.extractJaredDirectory("cloud/webhelp", PDFMojo.class, xslParentDirectory);
    }
		...
		pdfBuilder.setTocSectionDepth(getProperty("tocSectionDepth"));
		    ...
		    File targetDirForPdf = ((File)map.get("targetHtmlContentDir")).getParentFile();
        		if(!targetDirForPdf.exists()) {
        			org.openstack.docs.FileUtils.mkdir(targetDirForPdf);
        		}
